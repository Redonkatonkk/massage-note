...
当前版本：`0.6.0`。后续任何 AI 修补或开发都必须在同一次修改中迭代版本号；完整规则见[《AI 接管与修改指南》](docs/AI_HANDOFF.md)。
...
- 主要项目的多时长独立定价、主要/额外/折扣项目独立新增与排序、周一至周四门槛自动折扣，以及“员工项目 → 项目默认 → 员工默认 → 全店默认”四级提成。
- 全中文响应式今日表格、快速记工、同一员工多笔待结账、按最晚记工时间显示“空闲/下工时间”、手机自适应详情、非阻塞保存、排序、隐藏、软删除与恢复。
- 员工今日页隐藏全店经营汇总，历史营业日和个人日结均由服务端隔离为本人数据；个人日结可按设备屏幕生成 PNG 并通过手机分享菜单保存到相册。
