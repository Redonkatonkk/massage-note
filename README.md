...
当前版本：`0.7.2`。后续任何 AI 修补或开发都必须在同一次修改中迭代版本号；完整规则见[《AI 接管与修改指南》](docs/AI_HANDOFF.md)。
...
- 全中文响应式今日表格、快速记工、同一员工多笔待结账、项目时长变化时自动重算下工时间、按最晚记工时间显示“空闲/下工时间”、手机自适应详情、非阻塞保存、排序、集中管理隐藏员工（已日结日期也可恢复）、软删除与恢复。
