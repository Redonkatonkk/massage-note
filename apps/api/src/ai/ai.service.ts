...
export class AiService {
  ...
  private deterministicFinanceAnswer(summary: Awaited<ReturnType<FinanceQueriesService["summary"]>>, names: string[], method: string, amountType: string) {
    ...
    return `统计范围：${summary.filters.dateFrom} 至 ${summary.filters.dateTo}，${scope}，${methodText}，${typeText}。共 ${summary.totals.recordCount} 单；大费基数 ${money(summary.totals.grossFeeBaseCents)}，折后大费业绩 ${money(summary.totals.discountedFeePerformanceCents)}，实际收到大费 ${money(summary.totals.actualServiceCollectedCents)}，小费 ${money(summary.totals.totalTipCents)}，员工总收入 ${money(summary.totals.employeeIncomeCents)}，老板尚欠 ${money(summary.totals.employerOwesCents)}。这些金额均来自后端确定性财务引擎。`;
  }
