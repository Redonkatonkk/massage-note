...
export class FinanceAnalyticsService {
  ...
  async analytics(actor: User, storeId: string, query: FinanceAnalyticsQuery): Promise<FinanceAnalyticsResponse> {
    return this.prisma.$transaction(async client => {
      ...
      const [recordStart, saleStart, closingStart] = await Promise.all([
      ...
      const starts = [recordStart, saleStart, closingStart].flatMap(r => r._min.businessDate ? [dateOnly(r._min.businessDate)] : []).sort();
      ...
      const [records, sales, closings] = await Promise.all([
