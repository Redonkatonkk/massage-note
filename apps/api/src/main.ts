...
async function bootstrap(): Promise<void> {
  ...
  app.use((request: Request, response: Response, next: NextFunction) => {
    ...
    const isLangBotWorkRequest = (
      request.path === "/api/v1/integrations/langbot/work-context"
      || request.path === "/api/v1/integrations/langbot/work-events"
    ) && request.header("authorization")?.startsWith("Bearer mnw_");
    const origin = request.header("origin");
    if (!isSessionBootstrap && !isClosingDeliveryAgent && !isLangBotWorkRequest && origin !== webOrigin) {
