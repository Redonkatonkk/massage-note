...
async function heartbeat(lastError: string | null = null) {
  ...
  try {
    await request("/closing-delivery-agent/heartbeat", { method: "POST", body: JSON.stringify({ messagesAvailable: services.length > 0, serviceTypes: services.filter((item): item is "iMessage" | "RCS" | "SMS" => ["iMessage", "RCS", "SMS"].includes(item)), version: "1.0.4", lastError: diagnosticError }) });
  } catch (error) {
