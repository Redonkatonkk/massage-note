...
export async function renderClosingPng(snapshot: ClosingSnapshot, locale: Locale, svgPath: string, pngPath: string) {
  ...
  const labels = en ? {
    ...
    cash: "Cash", card: "Card", total: "Total", fee: "Service", tip: "Tip", recordIncome: "Earnings", footer: "Massage Note · Saved business-day snapshot",
  } : {
    title: "个人日结", service: "大费工资", tips: "小费工资", income: "今日总收入",
    base: "大费基数", submit: "应提交现金", records: "逐笔记工", confirmed: "已确认", pending: "待结账",
    cash: "现金", card: "刷卡", total: "合计", fee: "大费", tip: "小费", recordIncome: "本笔收入", footer: "Massage Note · 数据以系统保存的营业日快照为准",
  };
  const width = 1170;
  const recordHeight = 190;
  ...
  const rows = snapshot.records.map((record, index) => {
    ...
    const service = paymentAmounts(record.cashServiceCents, record.cardServiceCents, record.giftCardServiceCents, locale, 196, y + 105);
    const tips = paymentAmounts(record.cashTipCents, record.cardTipCents, record.giftCardTipCents, locale, 196, y + 148);
    return `<rect x="60" y="${y}" width="1050" height="170" rx="22" fill="#ffffff" fill-opacity=".92"/><text x="88" y="${y + 36}" class="record">${escapeXml(name)}</text><text x="88" y="${y + 66}" class="small">${escapeXml(time(record.startAt, snapshot.storeTimezone, locale))}–${escapeXml(time(record.endAt, snapshot.storeTimezone, locale))}</text><text x="840" y="${y + 36}" text-anchor="end" class="status">${record.status === "CONFIRMED" ? labels.confirmed : labels.pending}</text><line x1="870" y1="${y + 25}" x2="870" y2="${y + 145}" class="record-divider"/><text x="1080" y="${y + 72}" text-anchor="end" class="small">${escapeXml(labels.recordIncome)}</text><text x="1080" y="${y + 112}" text-anchor="end" class="amount">${escapeXml(money(record.employeeIncomeCents, locale))}</text><text x="88" y="${y + 105}" class="payment-label">${escapeXml(labels.fee)}：</text>${service}<text x="88" y="${y + 148}" class="payment-label">${escapeXml(labels.tip)}：</text>${tips}`;
  }).join("");
