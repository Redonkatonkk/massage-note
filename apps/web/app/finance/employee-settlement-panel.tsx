...
function DeliveryQueue({ value, busy, action }: { value: EmployeeSettlementDeliveryList; busy: boolean; action: (delivery: EmployeeSettlementDelivery, kind: "cancel" | "retry") => void }) {
  ...
  return <details className="settlement-delivery-queue"><summary>短信发送记录 <strong>{value.deliveries.length}</strong></summary><div className="table-scroll"><table className="data-table"><thead><tr><th>员工</th><th>区间</th><th>分类</th><th>附件进度</th><th>接收号码</th><th>尝试</th><th>错误</th><th>操作</th></tr></thead><tbody>{value.deliveries.map((item) => <tr key={item.id}><td>{item.membership.displayName}</td><td>{dateOnly(item.periodStart)} 至 {dateOnly(item.periodEnd)}</td><td>{scopeLabel(item.paymentScope)}</td><td><span className={`delivery-row-status is-${item.status.toLowerCase()}`}>{stage(item)}</span></td><td>{item.recipientPhoneE164}</td><td>{item.attemptCount}</td><td>{item.lastError || "—"}</td><td>{item.status === "QUEUED" ? <button className="table-action danger" type="button" disabled={busy} onClick={() => action(item, "cancel")}>取消</button> : item.status === "FAILED" ? <button className="table-action" type="button" disabled={busy} onClick={() => action(item, "retry")}>重试未完成附件</button> : "—"}</td></tr>)}</tbody></table></div></details>;
}
