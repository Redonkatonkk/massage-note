  ...
  return <section className="manage-section">
    <form className="manage-card" onSubmit={(event) => { event.preventDefault(); setSaved(false); void run(async () => {
      ...
      <section className={`auto-discount-settings${automaticDispatchEnabled ? " enabled" : ""}`}>
        <div className="auto-discount-heading"><div><strong>自动排工</strong><p>按员工最近一次名次生成每日顺序，并追踪点名、店里指定、画叉和欠位。</p></div><label className="inline-check"><input type="checkbox" disabled={!canManage} checked={automaticDispatchEnabled} onChange={(event) => setAutomaticDispatchEnabled(event.target.checked)} />开启</label></div>
        <p className="field-help">开启前必须先在成员管理中为所有参与记工的在职人员设置全职或兼职。关闭时保持原有手动记工流程。</p>
      </section>
...
function MembersPanel({ storeId, currentRole, members, requests, catalog, busy, run, reload }: { storeId: string; currentRole: MembershipSummary["role"]; members: StoreMember[]; requests: JoinRequest[]; catalog: CatalogResponse; busy: boolean; run: (action: () => Promise<void>) => Promise<void>; reload: () => Promise<void> }) {
  ...
  async function createEmployee() {
    await apiRequest(`/stores/${storeId}/members`, {
      method: "POST",
      body: { name: employeeName, employmentType },
    });
    setEmployeeName("");
    await reload();
  }
  return <section className="manage-section">
    <section className="manage-card"><div className="manage-heading"><div><p className="eyebrow">快速建档</p><h2>创建新员工</h2></div></div><p className="field-help">填写名字和排工类型。员工以后注册账号并用相同名字加入本店时，系统会自动关联这份资料和已有记工。</p><form className="inline-controls" onSubmit={(event) => { event.preventDefault(); void run(createEmployee); }}><label>员工名字<input required maxLength={80} value={employeeName} onChange={(event) => setEmployeeName(event.target.value)} /></label><label>排工类型<select value={employmentType} onChange={(event) => setEmploymentType(event.target.value as "FULL_TIME" | "PART_TIME")}><option value="FULL_TIME">全职</option><option value="PART_TIME">兼职</option></select></label><button className="primary-action compact" type="submit" disabled={busy || !employeeName.trim()}>{busy ? "正在创建…" : "创建员工"}</button></form></section>
...
function MemberEditor({ storeId, member, currentRole, catalog, busy, saved, refreshedToday, onDirty, onSaved, run, reload }: { storeId: string; member: StoreMember; currentRole: MembershipSummary["role"]; catalog: CatalogResponse; busy: boolean; saved: boolean; refreshedToday: boolean; onDirty: () => void; onSaved: (refreshedToday: boolean) => void; run: (action: () => Promise<void>) => Promise<void>; reload: () => Promise<void> }) {
  ...
  async function saveMember() {
    ...
    try {
      if (name.trim() !== member.displayName || role !== member.role || provider !== member.isServiceProvider || employmentType !== (member.employmentType ?? "") || closingDeliveryEnabled !== member.closingDeliveryEnabled || closingDeliveryPhone.trim() !== initialClosingDeliveryPhone || closingImageLocale !== (member.closingImageLocale ?? "")) {
        const updated = await apiRequest<StoreMember>(`/stores/${storeId}/members/${member.id}`, { method: "PATCH", body: { version, ...(isOwner ? {} : { displayName: name, role, isServiceProvider: provider }), employmentType: employmentType || null, closingDeliveryEnabled, closingDeliveryPhoneE164: closingDeliveryPhone.trim() || null, closingImageLocale: closingImageLocale || null } });
...
function CatalogPanel({ storeId, canManage, catalog, busy, run, reload }: { storeId: string; canManage: boolean; catalog: CatalogResponse; busy: boolean; run: (action: () => Promise<void>) => Promise<void>; reload: () => Promise<void> }) {
  ...
  return <section className="manage-section"><section className="manage-card">
    ...
    <CatalogGroup title="折扣项目" description="固定金额折扣由店铺承担，不会降低员工项目提成。" type="DISCOUNT" items={catalog.discountItems} emptyText="还没有折扣项目。" storeId={storeId} canManage={canManage} busy={busy} run={run} reload={reload} moveItem={moveItem} />
  </section></section>;
}
...
function CatalogCreateForm({ storeId, kind, busy, run, reload, onCreated }: { storeId: string; kind: CatalogKind; busy: boolean; run: (action: () => Promise<void>) => Promise<void>; reload: () => Promise<void>; onCreated: () => void }) {
  ...
  return <form className={`catalog-create catalog-create--${kind.toLowerCase()}`} onSubmit={(event) => { event.preventDefault(); void run(async () => {
    ...
    </div>)}<button className="secondary-action compact" type="button" onClick={() => setPriceOptions((current) => [...current, newPriceOption("")])}>＋ 添加时长价格</button></div> : <label className="catalog-field catalog-field--amount"><span>{kind === "DISCOUNT" ? "折扣金额（美元）" : "金额（美元）"}</span><input required aria-label={`${label}金额`} inputMode="decimal" placeholder="0.00" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>}
...
function CatalogItemEditor({ storeId, type, item, canManage, busy, run, reload, canMoveUp, canMoveDown, onMove }: { storeId: string; type: CatalogKind; item: ServiceItem | AddonItem | DiscountItem; canManage: boolean; busy: boolean; run: (action: () => Promise<void>) => Promise<void>; reload: () => Promise<void>; canMoveUp: boolean; canMoveDown: boolean; onMove: (direction: -1 | 1) => Promise<void> }) {
  ...
  function cancelEditing() {
    ...
    setAmount(formatMoneyInput(addon?.amountCents ?? discount?.amountCents ?? 0));
  ...
  return <article className={`catalog-item catalog-item--${type.toLowerCase()} ${deleted ? "deleted" : ""}`}>
    <div className="catalog-item-heading">
      <div className="catalog-summary">
        ...
        <div className="catalog-summary-facts">
          {service ? service.priceOptions.map((option) => <em key={option.id}>{option.durationMinutes} 分钟 · {money(option.priceCents)}</em>) : <em>{type === "DISCOUNT" ? `-${money(amountCents)}` : money(amountCents)}</em>}
    ...
    {canManage && !deleted && editing && <div className={`catalog-edit-panel catalog-edit-panel--${type.toLowerCase()}`}>
      ...
      </div> : <label className="catalog-field catalog-field--amount"><span>{discount ? "折扣金额（美元）" : "金额（美元）"}</span><input aria-label={`${item.shortName}金额`} inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>}
      ...
      <div className="catalog-form-actions"><button className="primary-action compact" disabled={busy} type="button" onClick={() => void run(async () => {
        const body = service
          ...
          : addon
            ...
            : { type, version: item.version, name, shortName, amountCents: parseMoney(amount, "折扣金额"), isEnabled };
