...
> 最后核对：2026-08-15（America/New_York）
> 当前版本：`0.7.1`
...
最近一次核心质量核对（2026-08-15，版本 0.7.1）：

- `version:check`、typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 114 项通过；数据库/API 集成测试 84 项通过，新增覆盖待认领成员迁移、经理只填名字创建员工、同名注册账号保留原 membership 自动认领，以及非同名账号不能靠加入表单冒领；原有财务、记工、日结与权限回归继续通过。
