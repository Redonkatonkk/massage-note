...
最近一次核心质量核对（2026-08-22，版本 0.12.15）：

- `version:check`、typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 164 项通过；数据库约束测试 4 项、API 全量测试 98 项通过。
- 全部迁移已在独立 `massage_note_test` 测试库部署；除礼物卡完整流程、高亮字段创建、三种财务筛选和 CSV 标记外，还覆盖页面整美元显示、整数付款摘要、安全的金额输入回填、自动与自定义礼物卡序列号防重、首页日结分流、记工卡片加项标志、无独立星标的高亮说明、开始时间和额外项目分钟数对结束时间的联动、卖卡收入／礼物卡核销支出在今日、财务与日结三处的一致性，以及每日流水公式。
- 财务每日小计已在中英文桌面和 390px 手机视口检查：今日流水位于营业日后的第一列，桌面完整显示，手机表格仅自身横向滚动且页面无横向溢出；记工卡片在手机端保持横向排列。今日记工顶部汇总在中英文和 390px 手机视口核对为五项，不显示小费总额且页面无横向溢出。
...
- 财务每日小计在营业日后第一项显示服务端整数美分计算的“今日流水 = 折后大费业绩 + 礼物卡销售收入 − 礼物卡核销支出”，并且不显示全部项目数、记工数、实收服务费、小费或客人总付款；其他汇总卡片和日结合计的同名指标保持不变。
...
- 财务汇总卡片不显示“老板尚欠”和“本期工资结算”，但不得删除累计余额和工资账本功能；其余 23 项（含礼物卡销售收入、核销支出、大费与小费）都要按用途分组并保留可悬停、聚焦和触屏点击的“！”解释与公式提示，卡片本身继续打开独立组成明细；明细宽表只能在弹窗内滚动，不能扩大页面宽度。店铺级卖卡记录不得被分摊到员工小计。
