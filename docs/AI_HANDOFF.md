...
> 最后核对：2026-08-22（America/New_York）
> 当前版本：`0.12.18`
...
员工总收入     = 大费工资 + 小费
今日流水       = 折后大费 + 礼物卡销售 - 礼物卡核销支出
总流水         = 折后大费 + 礼物卡销售 - 礼物卡核销支出
礼物卡收入     = 礼物卡销售 - 礼物卡核销支出
店铺收入       = 折后大费 + 小费 - 员工总收入 + 礼物卡销售 - 礼物卡核销支出
店铺总结算总收入 = 店铺收入 + 店长作为工人的收入 + 所有经理作为工人的收入 + 礼物卡收入
```

折扣由店铺承担，不减少员工大费工资。服务费差额允许存在，但必须提示并保存；已确认记录在首页只要 `paymentDifferenceCents !== 0` 就在卡片右下角显示红色“！”，该标记不读取小费。现金/刷卡/礼物卡大费至少填写一项；三种小费可以同时留空并按 0 处理，首页小费付款为零时只显示数字 `0`。记工详情只有一个“保存”按钮：待结账且付款字段完全空白时只执行详情 PATCH；任何付款或小费字段开始填写、勾选礼物卡，或记录原本已确认时，都先校验付款，再执行详情 PATCH 与付款确认两阶段操作。显式大费 `0` 算作已开始填写。第一阶段成功后前端必须立即采用响应中的最新 `version`，否则第二阶段失败后的重试会永久撞上旧版本冲突。使用礼物卡时必须保存序列号，未使用时序列号可以省略或为 `null`；礼物卡属于非现金付款，不增加员工现金结算。

每日卖卡使用独立 `gift_card_sales` 记录：新表单默认显示每店计数器从 1001 提供的建议号码，未修改时由服务端在事务内自动分配，也允许提交自定义序列号。序列号按 NFKC、大小写和空白规范化后同店防重，已软删除号码也不得用于新销售；`face_value_cents - discount_cents = amount_cents = cash_cents + card_cents > 0`。折扣使用售出时的门槛和比例快照，操作人必须是本店在职成员。实际收款全部计入店铺收入、不参与员工分成，也不进入员工现金结算；客人使用礼物卡支付的大费和小费全部计为店铺支出。`礼物卡核销支出 = 礼物卡大费 + 礼物卡小费`，`店铺收入 = 折后大费业绩 + 小费 - 员工总收入 + 礼物卡销售实际收款 - 礼物卡核销支出`。财务汇总默认全部付款方式，店铺级“全部项目数量”和“客人总付款”纳入礼物卡销售；员工筛选或仅大费、仅小费、仅高亮筛选不分摊店铺级卖卡记录。消费侧按规范化序列号汇总多条使用记录，但当前不维护或强制校验礼物卡余额，以兼容上线前已售卡。
...
最近一次核心质量核对（2026-08-23，版本 0.12.18）：

- `version:check`、typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 172 项通过；数据库约束测试 4 项、API 全量测试 98 项通过。
- 全部迁移已在独立 `massage_note_test` 测试库部署；除礼物卡完整流程、高亮字段创建、三种财务筛选和 CSV 标记外，还覆盖页面整美元显示、整数付款摘要、安全的金额输入回填、自动与自定义礼物卡序列号防重、首页日结分流、记工卡片加项标志、无独立星标的高亮说明、开始时间和额外项目分钟数对结束时间的联动、卖卡收入／礼物卡核销支出在今日、财务与日结三处的一致性、每日／总流水公式，以及店长、经理和礼物卡净收入的店铺总结算公式。
- 财务汇总已在中英文桌面和 390px 手机视口检查：每日小计位于员工小计之前，日期、星期和今日流水依次显示；员工筛选复选框可选择、清除并正确更新查询；关键成果与店铺总结算卡片完整显示，手机表格仅自身横向滚动且页面无横向溢出。今日记工顶部汇总仍为五项，不显示小费总额；记工卡片在手机端保持横向排列。
- 本地中英文浏览器验收已覆盖统一“保存”的空白付款提示和付款已填写提示，并真实确认一笔实收少于应收、空白小费的记录：首页小费付款显示 `0`，右下角显示红色“！”，英文无中文残留。项目设置验收确认新增/编辑默认收起，额外项目新增栏的名称、简称、金额、时间和默认提成在 1280px 视口同一水平线，紧凑金额/时间宽度分别为 96px/92px。
...
- 今日记工顶部的全店汇总不单独显示小费总额；店铺收入仍包含小费，员工行和财务页面中的小费指标保持不变。
- 财务每日小计必须位于员工小计之前，依次显示日期、按界面语言本地化的星期和服务端整数美分计算的“今日流水 = 折后大费业绩 + 礼物卡销售收入 − 礼物卡核销支出”，并且不显示全部项目数、记工数、实收服务费、小费或客人总付款。
...
- 记工详情修改开始时间时必须保持当前实际工作时长并同步移动结束时间；增加、移除或更换额外项目时按配置分钟数差值增减结束时间，主要项目时长变化也按差值处理。前端即时更新，服务端在未明确提交 `endAt` 时执行相同兜底；明确提交的结束时间始终优先。
- 记工详情只保留统一“保存”按钮：付款完全空白的待结账记录只保存详情；显式填写任一大费/小费、勾选礼物卡或重新保存已确认记录时必须校验并同步确认付款。首页红色“！”只标记已确认记录的服务费实收差额，小费为 0 只显示数字 `0` 且不得触发异常标记。
- 项目目录默认只显示摘要，新增与编辑按需展开；桌面宽度下额外项目的金额、时间、默认提成等字段必须保持紧凑横排，手机改为单列且页面不得横向溢出。
- 财务汇总共有 24 个卡片指标，关键成果不显示客人总付款或员工总收入，原“工资与现金结算”组不再显示；店铺总结算必须显示店铺收入、`OWNER` 作为工人的收入、全部 `MANAGER` 作为工人的收入、礼物卡销售减核销的净收入，以及前四项直接相加的总收入。累计余额和工资账本功能不得删除。所有卡片保留可悬停、聚焦和触屏点击的“！”解释与公式提示，卡片本身继续打开独立组成明细；明细宽表只能在弹窗内滚动，不能扩大页面宽度。员工范围使用复选框多选，空选择表示全部员工；店铺级卖卡记录不得被分摊到员工小计。
