...
最近一次核心质量核对（2026-08-22，版本 0.12.14）：

- `version:check`、typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 163 项通过；数据库约束测试 4 项、API 全量测试 98 项通过。
...
- 财务每日小计已在中英文桌面和 390px 手机视口检查：今日流水位于营业日后的第一列，桌面完整显示，手机表格仅自身横向滚动且页面无横向溢出；记工卡片在手机端保持横向排列。
