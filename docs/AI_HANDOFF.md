...
最近一次完整核对（2026-08-10，版本 0.3.0）：

- typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 98 项通过；数据库/API 集成测试 76 项通过。
- 生产依赖无已知漏洞；Compose 和 Shell 配置检查通过。
- standalone 生产服务主要路由、manifest 和 service worker HTTP 冒烟均为 200。
- 浏览器点击回归已覆盖店主手动加入今日表格后打卡、成功提示、实时变化不重复、三类项目独立新增与排序、390px 手机详情宽度锁定，以及记工保存后快速关闭。
