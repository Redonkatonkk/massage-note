...
最近一次核心质量核对（2026-08-16，版本 0.7.2）：
...
- 单元/非集成测试 115 项通过；数据库/API 集成测试 84 项通过，新增覆盖财务 16 项指标分组无遗漏、无重复，以及已日结营业日恢复隐藏员工且日结状态不变；原有待认领成员、记工、财务、日结与权限回归继续通过。
- 浏览器点击回归已覆盖隐藏员工后从集中管理区直接恢复、财务四组 16 张卡片完整显示、桌面 1440px 和手机 390px 点击卡片后页面宽度不变且组成明细只在弹窗内滚动；其余历史回归范围延续上一版本记录。
...
- 财务汇总卡片不显示“老板尚欠”和“本期工资结算”，但不得删除累计余额和工资账本功能；其余 16 项都要按用途分组并保留可悬停、聚焦和触屏点击的“！”解释与公式提示，卡片本身继续打开独立组成明细；明细宽表只能在弹窗内滚动，不能扩大页面宽度。
