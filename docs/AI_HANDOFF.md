...
折扣由店铺承担，不减少员工大费工资。服务费差额允许存在，但必须提示并保存。现金/刷卡/礼物卡大费至少填写一项；三种小费可以同时留空并按 0 处理。礼物卡付款必须保存序列号，礼物卡属于非现金付款，不增加员工现金结算。

每日卖卡使用独立 `gift_card_sales` 记录：`amount_cents = cash_cents + card_cents > 0`，同店有效序列号规范化后唯一，操作人必须是本店在职成员。卖卡总额全部计入店铺收入、不参与员工分成，也不进入员工现金结算；`店铺收入 = 折后大费业绩 + 小费 - 员工总收入 + 礼物卡销售总额`。消费侧当前只记序列号与本次大费/小费金额，不维护或强制校验礼物卡余额，以兼容上线前已售卡。
...
`0.10.0` 新增迁移 `20260820120000_gift_cards`，创建礼物卡销售表并扩展记工付款拆分与数据库金额约束。部署后需确认迁移已成功记录。
...
最近一次核心质量核对（2026-08-20，版本 0.10.0）：

- `version:check`、typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 132 项通过；数据库约束测试 4 项、API 集成测试 89 项通过。
- 礼物卡迁移已在独立 `massage_note_test` 测试库部署；覆盖卖卡幂等、同店序列号唯一、金额自动求和、今日收入、软删除/恢复，以及礼物卡大费/小费的确定性财务与现金隔离。
