...
每日卖卡使用独立 `gift_card_sales` 记录：序列号由每店计数器在事务内从 1001 自动分配且删除后不复用；`face_value_cents - discount_cents = amount_cents = cash_cents + card_cents > 0`。折扣使用售出时的门槛和比例快照，操作人必须是本店在职成员。实际收款全部计入店铺收入、不参与员工分成，也不进入员工现金结算；`店铺收入 = 折后大费业绩 + 小费 - 员工总收入 + 礼物卡销售实际收款`。消费侧按规范化序列号汇总多条使用记录，但当前不维护或强制校验礼物卡余额，以兼容上线前已售卡。
...
- 全店日结异常项必须在财务页原地打开单笔记工弹窗，不能把整个页面导航到今日页；今日页的“全店日结”只导航到财务日结标签，不得直接提交日结请求。
...
- 高亮按钮必须同时存在于快速记工和详情顶部，首页整张高亮卡使用黄色背景；财务高亮筛选不得只改前端列表，汇总、明细和 CSV 必须采用同一后端筛选。
