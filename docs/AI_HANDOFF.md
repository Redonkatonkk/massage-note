...
> 最后核对：2026-08-13（America/New_York）
> 当前版本：`0.6.13`
...
个人日结“应提交现金”与每日现金结算是两个口径：个人日结固定按所有含现金大费的已确认项目折前大费基数合计的 40% 计算，不减折扣，也不读取实际收到的现金大费作为金额基数；现金结算仍按真实现金流和实际取得工资核对。个人日结的现金大费分红等于已确认记工按现金付款比例分配的工资，刷卡大费分红等于剩余的刷卡付款对应工资；现金小费和刷卡小费分红分别汇总已确认的对应小费。混合付款沿用确认付款时的比例分摊，四项分红之和等于已确认记工的大费工资与小费合计。
...
最近一次核心质量核对（2026-08-13，版本 0.6.13）：

- `version:check`、typecheck 和生产构建通过；Next 生成 11 个路由。
- 单元/非集成测试 113 项通过；数据库/API 集成测试 81 项通过，覆盖财务汇总 16 项说明元数据、项目时长变化自动重算结束时间、员工上班把本人加入今日表格、个人日结按含现金大费项目折前基数的 40% 计算，以及已确认混合付款的现金／刷卡大费和小费四项分红汇总，同时保持现金结算实际现金流口径不变。
- 浏览器点击回归已覆盖财务汇总移除两张指定卡片、保留 16 个解释/公式入口且卡片仍能打开组成明细，记工从 60 分钟改为 90 分钟后界面即时更新结束时间且保存后今日下工时间同步、员工只显示“上班”且可把本人加入今日表格、不提供下班按钮、员工按当前记工显示“空闲/下工时间”、店长手动加入今日、全店日结入口准确传递店铺和营业日、日结异常逐笔返回并自动打开记工详情、实时变化不重复、三类项目独立新增与排序、390px 手机详情与快速记工宽度锁定及全局滚动边界、折扣 `off5`/`off10` 紧凑标记、桌面员工汇总可读性、员工当前页隐藏全店汇总、员工只读查看本人历史营业日、个人日结总收入旁的四项分红与独立现金交接、个人日结权限与图片生成、周一至周四自动折扣设置与记工应用，以及记工保存后快速关闭。
...
- 全店日结异常项的记工跳转必须同时携带店铺、营业日和记工编号；今日页的“全店日结”只导航到财务日结标签，不得直接提交日结请求。
- 记工主要项目时长变化时，结束时间必须按开始时间和新时长重算；前端即时更新，服务端在未明确提交 `endAt` 时兜底，不能继续沿用旧结束时间。
- 财务汇总卡片不显示“老板尚欠”和“本期工资结算”，但不得删除累计余额和工资账本功能；其余 16 项都要保留可悬停、聚焦和触屏点击的“！”解释与公式提示，卡片本身继续打开组成明细。
- Owner/Manager 可能参与记工；角色不等于工资对象。
- 现金小费与刷卡小费可同时为空；大费仍至少填一种。
- CSV 文本以 `= + - @` 开头时必须防公式注入。
- AI preview 未确认不写入，重复/并发确认只执行一次。
- 今日页只在普通员工尚未加入当前营业日表格时提供“上班”，用来建立本人班次和表格行；不提供“下班”。员工行状态仍只由当前时间是否落在记工时间段内决定。历史营业日需显式选择后查看；普通员工的历史表格必须在服务端缩小为本人数据，不能只靠前端隐藏他人行。
