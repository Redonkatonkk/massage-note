...
> 当前版本：`0.12.32` · 镜像：`ghcr.io/redonkatonkk/massage-note`
...
安装时必须在已登录“信息”的目标 macOS 用户桌面会话中运行 `scripts/install-messages-agent.sh`。首次运行安装附件暂存 App 后可能停止；店长只需在“完全磁盘访问权限”中添加并开启该 App，再重新运行安装。脚本随后通过 LaunchServices 以真正的无界面 App 身份验证 Messages 附件目录两级探针与自动化权限；不能从 LaunchAgent 直接执行 App 内二进制，否则 macOS 26 不会把 FDA 授权归给该 App。后台代理不使用模拟键盘、剪贴板或相册，也不读取聊天数据库。该用户退出登录后 LaunchAgent 不运行，重新登录会自动启动。向非 Apple 号码发送图片还依赖配对 iPhone 的短信转发和运营商 MMS/RCS 支持。
