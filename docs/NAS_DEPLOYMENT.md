...
镜像地址：`ghcr.io/redonkatonkk/massage-note`。当前版本：`0.12.16`。正常升级不再构建和上传约 250 MB 的 tar；`scripts/build-nas-image.sh` 只作为 GHCR 故障时的离线备用方案。
...
DOCKER_CONFIG="$task_docker_config" \
  docker manifest inspect ghcr.io/redonkatonkk/massage-note:0.12.16 >/dev/null
...
```bash
docker buildx imagetools inspect ghcr.io/redonkatonkk/massage-note:0.12.16
```
...
3. 设置 `MASSAGE_NOTE_IMAGE_TAG=0.12.16`、`APP_HTTP_PORT=3100`、`WEB_ORIGIN=https://massagenote.waltonjin.com`。
...
把 NAS `.env` 的 `MASSAGE_NOTE_IMAGE_TAG` 改为已经在 GHCR 验证存在的版本，例如 `0.12.16`。确认 `app`、`migrate`、`harden` 三个服务最终引用同一个标签，然后：
...
curl --fail https://massagenote.waltonjin.com/sw.js | grep 'massage-note-v0.12.16'
```
...
shasum -a 256 -c artifacts/massage-note-0.12.16-linux-amd64.tar.sha256
```
