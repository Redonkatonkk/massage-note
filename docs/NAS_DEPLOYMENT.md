...
镜像地址：`ghcr.io/redonkatonkk/massage-note`。当前版本：`0.2.1`。正常升级不再构建和上传约 250 MB 的 tar；`scripts/build-nas-image.sh` 只作为 GHCR 故障时的离线备用方案。
...
docker manifest inspect ghcr.io/redonkatonkk/massage-note:0.2.1 >/dev/null
```
...
```bash
docker buildx imagetools inspect ghcr.io/redonkatonkk/massage-note:0.2.1
```
...
3. 设置 `MASSAGE_NOTE_IMAGE_TAG=0.2.1`、`APP_HTTP_PORT=3100`、`WEB_ORIGIN=https://massagenote.waltonjin.com`。
...
把 NAS `.env` 的 `MASSAGE_NOTE_IMAGE_TAG` 改为已经在 GHCR 验证存在的版本，例如 `0.2.1`，然后：
...
curl --fail https://massagenote.waltonjin.com/sw.js | grep 'massage-note-v0.2.1'
```
...
shasum -a 256 -c artifacts/massage-note-0.2.1-linux-amd64.tar.sha256
```
