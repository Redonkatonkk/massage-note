...
- 日结与区间结算共用串行处理循环，每条任务结束后（成功或失败）冷却 60 秒后领取下一条；空队列仍每 10 秒轮询。冷却在领取下一条之前执行，不占用下一条任务的租约。
...
区间结算只记录单张长图的完成检查点。长图顶部放置店名、员工、结算区间和三张汇总卡，下面按营业日分组；每笔记工使用独立卡片，每天最后一张为“当日总结”。总结卡固定在当日最后一行最右列；两列布局遇到双数笔记工时在左侧保留一个空卡位，三列或四列布局也补足空位后右对齐。项目名称、付款拆分和混合付款提示按卡片宽度自动折行，不允许用省略号隐藏内容。同一天记工超过一行时自动换到下一行；渲染器会根据总天数和记录密度在 1080、1440、1680 像素源画布与 2～4 列之间选择布局，再在不超过 4 MB 的前提下压缩输出。图片不含独立封面、分页页眉或页码。
...
代理在渲染与暂存完成后、调用“信息”前再次校验租约和发送资格。日志采用临时文件加原子重命名；日志损坏或不可读时停止发送，需先恢复日志，不能当作空历史。API 请求有 30 秒超时，心跳只有在附件暂存和可识别发送账户均可用时才报告就绪。
...
- 仅在未开始发送且无法找到会话/收件人时尝试下一路由。一旦调用发送命令，异常按结果不确定处理，不自动换路由再次发送，需人工核对。
