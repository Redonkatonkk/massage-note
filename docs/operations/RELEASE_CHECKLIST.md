...
- [ ] 员工区间结算三种来源的汇总卡和逐笔金额正确；网页与短信长图均按营业日分组，每笔使用独立卡片、每天末尾显示“当日总结”，项目与付款文字完整自动折行。同日 18 笔和 31 天×每天 12 笔均可生成不超过 32,760 像素、4 MB 的单张长图，且不含独立封面、分页页眉或页码；API 第 1000 笔仍明确拒绝而不静默截断。
