...
- [ ] Mac“信息”代理通过 LaunchServices 以 FDA 授权 App 身份完成两级附件目录暂存并取得任务回执；个人日结任务只发送一张中英文 PNG、不附带文字气泡，员工区间结算依次发送摘要 PNG 与逐笔 PDF。个人日结背景在深色模式下不透明，工资按现金/刷卡拆分，现金交接位于逐笔之前，逐笔员工大费取折前基数，刷卡线框与礼物卡括号标识清晰。
- [ ] 成员短信接收号码会回填已关联账号的注册手机号；无注册手机号且未填专用号码时，开启接收无法保存并有明确提示。
- [ ] 员工日结短信队列可展开查看逐条详情；排队中的任务可由店主或经理取消，空号或非 E.164 历史任务不会阻塞后续发送。
- [ ] 员工区间结算三种来源的总结和逐笔金额正确；999 笔可预览并分页，1000 笔明确拒绝；短信依次收到摘要 PNG 和多页 PDF，PDF 失败重试不会重复已成功摘要。
