...
- [ ] 店铺总结算的信用卡手续费按普通刷卡金额 `2.5%`、高亮且含刷卡付款的记工每笔 `$3` 计算，部分刷卡按一笔，礼物卡和卖卡刷卡不计入，总收入正确扣减。
- [ ] 四级提成优先级与历史快照不随设置修改而变化。
- [ ] 周一至周四自动折扣只在折前大费达到门槛时应用，可与手动折扣并存，并且不减少员工大费工资。
- [ ] Employee 今日页不显示全店汇总，历史营业日和个人日结只返回本人数据；个人日结图片可在目标手机生成并保存。
- [ ] Mac“信息”代理通过 LaunchServices 以 FDA 授权 App 身份完成两级附件目录暂存并取得任务回执；个人日结任务只发送一张中英文 PNG、不附带文字气泡，员工区间结算只发送一张顶部含三张汇总卡的逐笔 JPEG 长图。个人日结背景在深色模式下不透明，工资按现金/刷卡拆分，现金交接位于逐笔之前，逐笔员工大费取折前基数，刷卡线框与礼物卡括号标识清晰。
- [ ] 成员短信接收号码会回填已关联账号的注册手机号；无注册手机号且未填专用号码时，开启接收无法保存并有明确提示。
- [ ] 员工日结短信队列可展开查看逐条详情；排队中的任务可由店主或经理取消，空号或非 E.164 历史任务不会阻塞后续发送。
- [ ] 员工区间结算三种来源的汇总卡和逐笔金额正确，小数金额按美分显示；短信发送记录由发送按钮旁的弹窗打开。网页与短信长图均按营业日分组，每笔使用独立卡片；长图“当日总结”固定在最右列，双数笔记工时左侧留空，项目与付款文字完整自动折行。同日 18 笔和 31 天×每天 12 笔均可生成不超过 32,760 像素、4 MB 的单张长图，且不含独立封面、分页页眉或页码；API 第 1000 笔仍明确拒绝而不静默截断。
...
- [ ] 每日开门排位关闭时原有记工与手动排序不变；开启前必须补齐所有在职记工成员的全职/兼职类型。
- [ ] 当前未日结营业日可生成、重新生成和手动调整员工顺序；普通员工、历史营业日、已日结营业日和跨店请求均无法生成或修改排位。
- [ ] 排位只改变员工顺序；普通记工、AI 记工、编辑、删除、恢复、付款和日结均不创建或读取逐工派工状态。
